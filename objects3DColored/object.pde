...
class Object3D {
  ...
  public void desenhaPoligono() {
    ...
    for (int i = 0; i < faces; i++) {  
      ...
      ni[i] = nx*(PO[projecao][X] - P[1][X]) + ny*(PO[projecao][Y] - P[1][Y]) + nz*(PO[projecao][Z] - P[1][Z]);
    ...
    for(int i = 0; i < faces; i++) {
      if (ni[i] > 0) {
        float[][] faceVertices = new float[mFaces[i].size()][2];
        for(int j = 0; j < mFaces[i].size(); j++) {
          faceVertices[j][X] = auxPontos[mFaces[i].get(j)] [X];
          faceVertices[j][Y] = auxPontos[mFaces[i].get(j)] [Y];
        }

        for(int j = 0; j < faceVertices.length; j++) {
          if (j == faceVertices.length-1) {
            linhaDDA(int(faceVertices[j][0]), int(faceVertices[j][1]), int(faceVertices[0][0]), int(faceVertices[0][1]), this.corLinha);  
          } else {
            linhaDDA(int(faceVertices[j][0]), int(faceVertices[j][1]), int(faceVertices[j+1][0]), int(faceVertices[j+1][1]), this.corLinha);
          }
        }
      }
    }      
